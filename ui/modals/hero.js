...
export function openAppInfoModal() {
    ...
    const teacherDailyFlow = [
        { emoji: '🏫', title: 'Open the class (2 min)', body: 'Follow today’s schedule is already on when you open the app: the class in session, or General view when none is. You can still pick a class by hand. Glance at Home. Open Projector Mode whenever the room should see the wallpaper — sky, clocks, story cards. It is not a step you must do first.' },
        ...
        { emoji: '🏆', title: 'Review & Celebrate', body: 'Glance at Team Quest and Hero\'s Challenge. When a new school month begins, run the Ceremony of the Month — Growth Festival for Nursery and Pre-Junior, Classic Arena (classes, then Prodigy) for the rest. August is closed, so there is no August ceremony. Students leave talking about the Quest — that is the goal!' },
    ];
